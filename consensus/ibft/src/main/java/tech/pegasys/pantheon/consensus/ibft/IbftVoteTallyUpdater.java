...
public class IbftVoteTallyUpdater implements VoteTallyUpdater {
  ...
  public void updateForBlock(final BlockHeader header, final VoteTally voteTally) {
    ...
    Optional<Vote> optionalVote = ibftExtraData.getVote();

    optionalVote.ifPresent(
        vote -> {
          Address recipient = vote.getRecipient();

          if (!recipient.equals(NO_VOTE_SUBJECT)) {
            final Address proposer = header.getCoinbase();
            voteTally.addVote(proposer, recipient, vote.getVoteType());
          }
        });
  }
}
